...
%  algs=algs0(keep);
  ...
  dts = loadDt( algs, plotName, aspectRatio );%dt????bnds
...
for g=1:nGt
  for d=1:nDt%xs = fp/nImg ys tp/np miss rate = 1-ys
    [xs{g,d},ys{g,d},~,score] = ...
      bbGt('compRoc',res(g,d).gtr,res(g,d).dtr,plotRoc,samples);
    if(plotRoc), ys{g,d}=1-ys{g,d}; score=1-score; end%score is consum, while ys is not.this score is ref
...
%  savefig(fName1,1,'png','-r300','-fonts'); close(1); 
 % f1=[fName1 '.pdf'];
...
fName=@(s,v) sprintf('%s/annotations/set%02i/V%03i',pth1,s,v);
A=vbb('vbbLoad',fName(s,v));
end
...
for i=1:nExp
  ...
  for s=1:length(setIds)
    for v=1:length(vidIds{s})
      A = loadVbb(setIds(s),vidIds{s}(v));
      for f=skip-1:skip:A.nFrame-1
        bb = vbb('frameAnn',A,f+1,lbls,filterGt); ids=bb(:,5)~=1;
        bb(ids,:)=bbApply('resize',bb(ids,:),1,0,aspectRatio);%?????????aspectRatio????
        k=k+1; gt{k}=bb;
      end
    end
  end
...
for i=1:nAlg
  ...
  dt=cell(1,100000); k=0; aDir=[dbInfo '/video_res/' algs(i).name];
  if(algs(i).resize), resize=100/128; else resize=1; end
  for s=1:length(setIds), s1=setIds(s);
    for v=1:length(vidIds{s}), v1=vidIds{s}(v);
      A=loadVbb(s1,v1); frames=skip-1:skip:A.nFrame-1;
      vName=sprintf('%s/detection',aDir);
     % vName = 'detection';
